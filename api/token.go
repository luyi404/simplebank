...
func (server *Server) renewAccessTokens(ctx *gin.Context) {
	...
	if session.Username != refreshPayload.Username {
		err := fmt.Errorf("username is not matched")
		ctx.JSON(http.StatusUnauthorized, errorResponse(err))
		return
	}

	if session.RefreshToken != req.RefreshToken {
		err := fmt.Errorf("refresh token is not matched")
		ctx.JSON(http.StatusUnauthorized, errorResponse(err))
		return
	}
